...
// CHECK: %[[LOOP:[0-9]+]] = scf.forall (%{{.*}}, %{{.*}}, %{{.*}}) in (2, 2, 8) shared_outs(%{{.*}} = %{{.*}}) -> (tensor<2x2x1024x64xf16>)
...
// CHECK: tensor.parallel_insert_slice %{{.*}} into %{{.*}}[%{{.*}}, %{{.*}}, %{{.*}}, 0] [1, 1, 128, 64] [1, 1, 1, 1] : tensor<128x64xf16> into tensor<2x2x1024x64xf16>
// CHECK: return %{{.*}} : tensor<1x2x2x1024x64xf16>
