...
// CHECK: %[[INIT:[0-9]+]] = tensor.empty() : tensor<1x2x2x1024x64xf16>
// CHECK: %[[LOOP:[0-9]+]] = scf.forall (%{{.*}}, %{{.*}}, %{{.*}}) in (2, 2, 8) shared_outs(%{{.*}} = %[[INIT]]) -> (tensor<1x2x2x1024x64xf16>)
// CHECK: htile.full %{{.*}} : f32 -> tensor<128xf32>
// CHECK: htile.full %{{.*}} : f32 -> tensor<128x64xf32>
// CHECK: %{{.*}}:3 = scf.for %{{.*}} = %c0 to %c16 step %c1 iter_args(
// CHECK: affine.apply
// CHECK: affine.apply
// CHECK: tensor.extract_slice %arg0[0, %{{.*}}, %{{.*}}, 0] [1, 1, 128, 64] [1, 1, 1, 1]
...
// CHECK: tensor.parallel_insert_slice %{{.*}} into %{{.*}}[0, %{{.*}}, %{{.*}}, %{{.*}}, 0] [1, 1, 1, 128, 64] [1, 1, 1, 1, 1] : tensor<128x64xf16> into tensor<1x2x2x1024x64xf16>
// CHECK: return %[[LOOP]] : tensor<1x2x2x1024x64xf16>
