...
module @jit_selective_scan attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32, transform.with_named_sequence} {
  transform.named_sequence @__transform_main(%module: !any) {
    ...
    // Generalize some linalg ops because `linalg.fold_unit_extent_dims_via_reshapes`
    // only works with linalg.generic.
    %maps = transform.structured.match ops{["linalg.map", "linalg.broadcast"]} in %func
    : (!transform.any_op) -> !transform.any_op
    transform.structured.generalize %maps : (!transform.any_op) -> !transform.any_op
...
// CHECK: %[[STATE_NEXT:.+]] = arith.addf {{.*}} : tensor<128x16xf32>
