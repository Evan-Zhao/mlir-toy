...
//
// Transform-dialect schedule that transforms the Torch-MLIR MQA payload below
// into a FlashAttention-like fused program with a `(group, unit-head)` outer loop shape.
...
module attributes {transform.with_named_sequence} {
  ...
  func.func @attention(%arg0: tensor<1x4x1024x128xf16>, %arg1: tensor<1x1x1024x128xf16>, %arg2: tensor<1x1x1024x128xf16>) -> tensor<1x4x1x1024x128xf16> {
    %cst = stablehlo.constant dense<0xFF800000> : tensor<f32>
    %cst_0 = stablehlo.constant dense<0.000000e+00> : tensor<f32>
    %cst_1 = arith.constant dense<0.088388347648318433> : tensor<1xf64>
    %0 = stablehlo.reshape %arg0 : (tensor<1x4x1024x128xf16>) -> tensor<1x4x1x1024x128xf16>
    %1 = stablehlo.reshape %arg1 : (tensor<1x1x1024x128xf16>) -> tensor<1x1x1x1024x128xf16>
    %2 = stablehlo.broadcast_in_dim %1, dims = [0, 1, 2, 3, 4] : (tensor<1x1x1x1024x128xf16>) -> tensor<1x4x1x1024x128xf16>
    %3 = stablehlo.reshape %arg2 : (tensor<1x1x1024x128xf16>) -> tensor<1x1x1x1024x128xf16>
    %4 = stablehlo.broadcast_in_dim %3, dims = [0, 1, 2, 3, 4] : (tensor<1x1x1x1024x128xf16>) -> tensor<1x4x1x1024x128xf16>
    %5 = stablehlo.convert %0 : (tensor<1x4x1x1024x128xf16>) -> tensor<1x4x1x1024x128xf32>
    %6 = stablehlo.convert %2 : (tensor<1x4x1x1024x128xf16>) -> tensor<1x4x1x1024x128xf32>
    %7 = stablehlo.transpose %6, dims = [0, 1, 2, 4, 3] : (tensor<1x4x1x1024x128xf32>) -> tensor<1x4x1x128x1024xf32>
    %8 = stablehlo.broadcast_in_dim %7, dims = [0, 1, 2, 3, 4] : (tensor<1x4x1x128x1024xf32>) -> tensor<1x4x1x128x1024xf32>
    %9 = stablehlo.dot_general %5, %8, batching_dims = [0, 1, 2] x [0, 1, 2], contracting_dims = [4] x [3] : (tensor<1x4x1x1024x128xf32>, tensor<1x4x1x128x1024xf32>) -> tensor<1x4x1x1024x1024xf32>
    %10 = stablehlo.convert %cst_1 : (tensor<1xf64>) -> tensor<1xf32>
    %11 = stablehlo.reshape %10 : (tensor<1xf32>) -> tensor<f32>
    %12 = stablehlo.broadcast_in_dim %11, dims = [] : (tensor<f32>) -> tensor<1x4x1x1024x1024xf32>
    %13 = stablehlo.multiply %9, %12 : tensor<1x4x1x1024x1024xf32>
    %14 = stablehlo.reduce(%13 init: %cst) applies stablehlo.maximum across dimensions = [4] : (tensor<1x4x1x1024x1024xf32>, tensor<f32>) -> tensor<1x4x1x1024xf32>
    %15 = stablehlo.reshape %14 : (tensor<1x4x1x1024xf32>) -> tensor<1x4x1x1024x1xf32>
    %16 = stablehlo.broadcast_in_dim %15, dims = [0, 1, 2, 3, 4] : (tensor<1x4x1x1024x1xf32>) -> tensor<1x4x1x1024x1024xf32>
    %17 = stablehlo.subtract %13, %16 : tensor<1x4x1x1024x1024xf32>
    %18 = stablehlo.exponential %17 : tensor<1x4x1x1024x1024xf32>
    %19 = stablehlo.reduce(%18 init: %cst_0) applies stablehlo.add across dimensions = [4] : (tensor<1x4x1x1024x1024xf32>, tensor<f32>) -> tensor<1x4x1x1024xf32>
    %20 = stablehlo.reshape %19 : (tensor<1x4x1x1024xf32>) -> tensor<1x4x1x1024x1xf32>
    %21 = stablehlo.broadcast_in_dim %20, dims = [0, 1, 2, 3, 4] : (tensor<1x4x1x1024x1xf32>) -> tensor<1x4x1x1024x1024xf32>
    %22 = stablehlo.divide %18, %21 : tensor<1x4x1x1024x1024xf32>
    %23 = stablehlo.convert %4 : (tensor<1x4x1x1024x128xf16>) -> tensor<1x4x1x1024x128xf32>
    %24 = stablehlo.broadcast_in_dim %23, dims = [0, 1, 2, 3, 4] : (tensor<1x4x1x1024x128xf32>) -> tensor<1x4x1x1024x128xf32>
    %25 = stablehlo.dot_general %22, %24, batching_dims = [0, 1, 2] x [0, 1, 2], contracting_dims = [4] x [3] : (tensor<1x4x1x1024x1024xf32>, tensor<1x4x1x1024x128xf32>) -> tensor<1x4x1x1024x128xf32>
    %26 = stablehlo.convert %25 : (tensor<1x4x1x1024x128xf32>) -> tensor<1x4x1x1024x128xf16>
    %27 = stablehlo.reshape %26 : (tensor<1x4x1x1024x128xf16>) -> tensor<1x4x1x1024x128xf16>
    return %27 : tensor<1x4x1x1024x128xf16>
  }
}

// CHECK-LABEL: func.func @attention(
// CHECK-SAME: %arg0: tensor<1x4x1024x128xf16>, %arg1: tensor<1x1x1024x128xf16>, %arg2: tensor<1x1x1024x128xf16>) -> tensor<1x4x1x1024x128xf16>
// CHECK-NOT: stablehlo
// CHECK-NOT: linalg.batch_matmul
// CHECK-NOT: linalg.transpose
// CHECK-NOT: linalg.generic
// CHECK: %[[OUT:.*]] = memref.alloc() : memref<1x4x1x1024x128xf16>
// CHECK: %[[Q:.*]] = bufferization.to_buffer %arg0 read_only : tensor<1x4x1024x128xf16> to memref<1x4x1024x128xf16>
// CHECK: %[[K:.*]] = bufferization.to_buffer %arg1 read_only : tensor<1x1x1024x128xf16> to memref<1x1x1024x128xf16>
// CHECK: %[[V:.*]] = bufferization.to_buffer %arg2 read_only : tensor<1x1x1024x128xf16> to memref<1x1x1024x128xf16>
...
// CHECK-SAME: memref<1x4x1024x128xf16>
// CHECK-SAME: memref<1x1x1024x128xf16>
// CHECK-SAME: memref<1x1x1024x128xf16>
// CHECK-SAME: memref<1x4x1x1024x128xf16>
// CHECK-SAME: attributes {program_bounds = array<i64: 4, 8>}
// CHECK: htile.program_id 0
// CHECK: htile.program_id 1
// CHECK-NOT: htile.program_id 2
// CHECK-NOT: scf.forall
