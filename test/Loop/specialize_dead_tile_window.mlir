...
module attributes {transform.with_named_sequence} {
  transform.named_sequence @__transform_main(%module: !any) {
    %func = transform.structured.match ops{["func.func"]}
        attributes {sym_name = "windowed_dead_tile"} in %module : (!any) -> !any
  ...
  // CHECK-LABEL: func.func @windowed_dead_tile(
  ...
  func.func @windowed_dead_tile(%q_block: index, %live: tensor<1x128x64xf32>)
    ...
    %result = scf.for %j = %c0 to %c16 step %c1
      ...
      ^bb0(%in: f32, %dead_scalar: f32, %out: f32):
        ...
        %k_plus_window = arith.addi %k_i64, %c64_i64 : i64
        %not_future = arith.cmpi sle, %k_i64, %q_i64 : i64
        %within_window = arith.cmpi slt, %q_i64, %k_plus_window : i64
        %is_live = arith.andi %not_future, %within_window : i1
