...
def test_lowered_attention_mlir_is_triton_translator_input() -> None:
    ...
    with tempfile.NamedTemporaryFile("w", suffix=".mlir", delete=False) as f:
        f.write(lowered)
        lowered_path = Path(f.name)

    result = subprocess.run(
        [
            sys.executable,
            "-m",
            "neptune_mlir.translators.triton",
            str(lowered_path),
            "--plugin",
            str(plugins.htile_dialect),
        ],
        capture_output=True,
        text=True,
    )
    ...
    assert "@triton.jit" in result.stdout
    assert "def attention" in result.stdout
