"""Export a JAX Mamba selective scan as StableHLO MLIR.
...
def selective_scan(
    ...
    # The loop index is in bounds, so dynamic slice clamping is never exercised.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sequence-length", type=_positive_int, default=2048)
    parser.add_argument("--model-dim", type=_positive_int, default=768)
    parser.add_argument("--expand", type=_positive_int, default=2)
    parser.add_argument("--state-dim", type=_positive_int, default=16)
    parser.add_argument(
...
def main() -> None:
    ...
    u = jax.ShapeDtypeStruct((args.batch, args.sequence_length, channels), activation_dtype)
    delta = jax.ShapeDtypeStruct(u.shape, activation_dtype)
    a = jax.ShapeDtypeStruct((channels, args.state_dim), jnp.float32)
    b = jax.ShapeDtypeStruct((args.batch, args.sequence_length, args.state_dim), activation_dtype)
    c = jax.ShapeDtypeStruct(b.shape, activation_dtype)
    d_skip = jax.ShapeDtypeStruct((channels,), jnp.float32)
    delta_bias = jax.ShapeDtypeStruct((channels,), jnp.float32)

    module_text = selective_scan.lower(u, delta, a, b, c, d_skip, delta_bias).as_text()
